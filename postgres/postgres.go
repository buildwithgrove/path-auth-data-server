...
*/
func NewPostgresDataSource(ctx context.Context, connectionString string, logger polylog.Logger) (*postgresDataSource, error) {
...
func (d *postgresDataSource) FetchAuthDataSync() (*proto.AuthDataResponse, error) {
	...
	rows, err := d.driver.SelectGatewayEndpoints(context.Background())
	if err != nil {
		d.logger.Error().Err(err).Msg("failed to get gateway endpoints")
		return nil, err
	}

	return convertGatewayEndpointsRows(rows), nil
}
...
func (d *postgresDataSource) processPortalApplicationChanges(ctx context.Context) error {
	...
	changes, err := d.driver.GetGatewayEndpointChanges(ctx)
	if err != nil {
		return err
	}
	...
	for _, change := range changes {
		...
		if change.IsDelete {
			update := &proto.AuthDataUpdate{
				EndpointId: change.GatewayEndpointID,
				Delete:     true,
			}
			d.updatesCh <- update
		} else {
			gatewayEndpointRow, err := d.driver.SelectGatewayEndpoint(ctx, change.GatewayEndpointID)
			if err != nil {
				d.logger.Error().Err(err).Msg("failed to get gateway endpoint")
				continue
			}

			gatewayEndpoint := convertSelectGatewayEndpointRow(gatewayEndpointRow).convertToProto()
	...
	err = d.driver.DeleteGatewayEndpointChanges(ctx, changeIDs)
	if err != nil {
		return err
	}
